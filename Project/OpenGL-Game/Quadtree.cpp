...
int Quadtree::getIndex(LocationRect pRect)
{
	int index = -1;
	double verticalMidpoint = bounds.sx + (bounds.getWidth() / 2);
	double horizontalMidpoint = bounds.sy + (bounds.getHeight() / 2);
...
int Quadtree::getIndex(RigidBody* pRect)
{
	int index = -1;
	double verticalMidpoint = bounds.sx + (bounds.getWidth() / 2);
	double horizontalMidpoint = bounds.sy + (bounds.getHeight() / 2);
	...
	// Object can completely fit within the top quadrants
	bool topQuadrant = (pRect->pMesh->bottomFace.sy < horizontalMidpoint && pRect->pMesh->bottomFace.sy + pRect->pMesh->bottomFace.getHeight() < horizontalMidpoint);
	// Object can completely fit within the bottom quadrants
	bool bottomQuadrant = (pRect->pMesh->bottomFace.sy > horizontalMidpoint);

	// Object can completely fit within the left quadrants
	if (pRect->pMesh->bottomFace.sx < verticalMidpoint && pRect->pMesh->bottomFace.sx + pRect->pMesh->bottomFace.getWidth() < verticalMidpoint) {
		if (topQuadrant) {
			index = 1;
		}
		else if (bottomQuadrant) {
	...
	// Object can completely fit within the right quadrants
	else if (pRect->pMesh->bottomFace.sx > verticalMidpoint) {
		if (topQuadrant) {
			index = 0;
		}
		else if (bottomQuadrant) {
...
// Object can completely fit within the top quadrants
bool topQuadrant = (pRect.rect.sy < horizontalMidpoint && pRect.rect.sy + pRect.rect.getHeight() < horizontalMidpoint);
// Object can completely fit within the bottom quadrants
bool bottomQuadrant = (pRect.rect.sy > horizontalMidpoint);

// Object can completely fit within the left quadrants
if (pRect.rect.sx < verticalMidpoint && pRect.rect.sx + pRect.rect.getWidth() < verticalMidpoint) {
if (topQuadrant) {
index = 1;
}
else if (bottomQuadrant) {
...
// Object can completely fit within the right quadrants
else if (pRect.rect.sx > verticalMidpoint) {
if (topQuadrant) {
index = 0;
}
else if (bottomQuadrant) {
...
void Quadtree::insert(RigidBody *pRect)
{
	if (nodes != nullptr)
	{
		if (nodes[0] != nullptr) {
			int index = getIndex(pRect->pMesh->bottomFace);
	...
	rigidObjects.push_back(pRect);

	if (rigidObjects.size() > MAX_OBJECTS && level < MAX_LEVELS) {
		if (nodes == nullptr) {
			split();
		}//*/
		...
		int i = 0;
		while (i < rigidObjects.size()) {
			int index = getIndex(rigidObjects.at(i));
			if (index != -1) {
				nodes[index]->insert(rigidObjects[i]);
...
list<RigidBody*>* Quadtree::retrieve(list<RigidBody*> *returnObjects, RigidBody* pRect)
{
	int index = getIndex(pRect->pMesh->bottomFace);
	...
	//returnObjects.addAll(objects);
	for (deque<RigidBody*>::iterator it = rigidObjects.begin(); it != rigidObjects.end(); ++it)
	{
		returnObjects->push_back(*it);
	}
	return returnObjects;
